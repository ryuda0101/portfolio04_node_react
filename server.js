...
app.use(express.static(path.join(__dirname, 'portfolio04/build')));

app.get('*', function (req, res) {
    res.sendFile(path.join(__dirname, '/portfolio04/build/index.html'));
  });
...
// 상세페이지(detail 페이지) 뒤에 번호 붙여서 특정 번호의 게시글 상세페이지로 이동하는 코드
// 1. get요청으로 상세페이지를 부른다. 이때 주소 뒤에 /:를 쓰고 원하는 이름을 설정해준다. 
// app.get("/detail/:no",(req,res) => {
// 2. db의 컬렉션에서 상세페이지에서 보여줄 게시글의 번호를 찾아주고, 그 번호를 위의 주소에 붙여준다.
//    찾아온 정보는 객체의 형태로 가져오도록 한다.
//    이때 주의할점은 req.params.no로 가져온 정보는 string이기 때문에!! 꼭!!! Number로 바꿔줘야 한다!
// db.collection("board").findOne({num:Number(req.params.no)}).toArray((err,result) => {
// 3. 이후 상세페이지.ejs에 db의 컬렉션에서 가져온 result값을 보내준다.
...
// db에 moment로 작성 시간 올릴때 다음과 같이 써준다.
...
// 이때 수정form에 파일첨부 input이 있다면 다음의 코드를 경로와 function 사이에 꼭!!! 써준다!
// ,upload.single('brd_file'),
// ex. 
...
//     db.collection('아이디, 비밀번호가 들어있는 컬렉션 이름').findOne({ 컬렉션에서 아이디가 들어있는 데이터 이름: 위에서작명한입력한아이디 }, function (err, result) {
...
// “/경로”와 function 사이에 꼭 upload.single('ejs에서 가져온 file input태그의 name값')를 넣어줘야 한다!!
// 
// 5.5. 조건문으로 첨부파일이 들어가지 않으면 null 값으로 db에 넣어주기
// if (req.file) {
...
// }
//
// app.post("/add",upload.single('ejs에서 가져온 file input태그의 name값'),function(req,res){
//     db.collection("컬렉션 이름").insertOne({
//         fileName:fileUpload;
//     },function(err,result){
//         res.redirect("/다시 돌아갈 경로");
//     });
// });
